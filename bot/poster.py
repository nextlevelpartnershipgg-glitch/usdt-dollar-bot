...
MAX_POSTS_PER_RUN  = int(os.environ.get("MAX_POSTS_PER_RUN", "1"))    # 1 за прогон → ~5/час
...
# ====== ИСТОЧНИКИ (≥30, РФ-фокус; можно пополнять) ======
RSS_FEEDS_RU = [
    ...
    "https://www.fontanka.ru/fontanka.rss",         # регионы
    "https://www.komi24.ru/rss",                    # регионы (пример)
    "https://gov.ru/rss",                           # правительство (если доступно)
    "https://minfin.gov.ru/ru/press-center/?rss=Y",# Минфин
    "https://cbr.ru/StaticHtml/Rss/Press",          # Банк России (пресс-релизы)
    "https://moscowexchange.com/Export/MRSS/News",  # Мосбиржа (если доступно)
    "https://1prime.ru/export/rss2/index.xml",      # Прайм
    "https://www.kommersant.ru/RSS/money.xml",
    "https://rg.ru/tema/ekonomika/rss.xml",         # РГ экономика
    "https://www.ng.ru/rss/",                       # Независимая (общая)
    "https://www.mk.ru/rss/finance/index.xml",      # МК финансы
    "https://tass.ru/economy/rss",                  # ТАСС экономика
    "https://tass.ru/politika/rss",                 # ТАСС политика
    "https://www.forbes.ru/newrss.xml",             # Forbes RU
    "https://www.rbc.ru/story/rss/5e9d954f9a7947a3aaf0c9d5",  # сюжеты
]
RSS_FEEDS_WORLD = [
    ...
    "https://www.bloomberg.com/feed/podcast/etf_report.xml",   # тех. лента
...
def random_gradient(w=1080, h=540):
    top,bottom=random.choice(PALETTES); top=_boost(top,1.3); bottom=_boost(bottom,1.3)
    angle=random.choice([0,15,30,45,60,75,90,120,135])
    img=Image.new("RGB",(w,h)); d=ImageDraw.Draw(img); steps=max(w,h)
    for i in range(steps):
        t=i/(steps-1); r=int(top[0]*(1-t)+bottom[0]*t); g=int(top[1]*(1-t)+bottom[1]*t); b=int(top[2]*(1-t)+bottom[2]*t)
        d.line([(i*w//steps,0),(i*w//steps,h)], fill=(r,g,b))
    if angle not in (90,270): img=img.rotate(angle, expand=False, resample=Image.BICUBIC)
    img=ImageEnhance.Contrast(img).enhance(1.15); img=ImageEnhance.Brightness(img).enhance(1.05)
    mask=Image.new("L",(w,h),0); md=ImageDraw.Draw(mask); md.ellipse([-w*0.2,-h*0.4,w*1.2,h*1.4], fill=210)
    mask=mask.filter(ImageFilter.GaussianBlur(radius=80)); img=Image.composite(img, Image.new("RGB",(w,h),(0,0,0)), mask)
    return img
...
def ru_scientific_paraphrase(s):
    ...
    out=re.sub(r"\s+%","%",out); return re.sub(r"\s+"," ",out).strip()
def split_sentences(text): text=re.sub(r"\s+"," ",text or "").strip(); return re.split(r"(?<=[.!?])\s+", text) if text else []
def paraphrase_sentence_ru_or_en(s): 
    if detect_lang(s)=="en": s=translate_hard_ru(s)
...
def fetch_article_text(url, max_chars=2600):
    try:
        ...
        soup=BeautifulSoup(r.text,"html.parser"); ps=soup.find_all("p"); chunks=[]
        for p in ps:
            t=p.get_text(" ",strip=True)
            if not t or len(t)<60: continue
            if any(x in t.lower() for x in ["javascript","cookie","подпишитесь","реклама","cookies"]): continue
            chunks.append(t); if sum(len(c) for c in chunks)>max_chars: break
...
def build_three_paragraphs_scientific(title, article_text, feed_summary):
    ...
    base_ru=ensure_russian(base_raw); sents=[s for s in split_sentences(base_ru) if s]
    p1_src=sents[:2] or sents[:1]; p2_src=sents[2:5] or sents[:1]; p3_src=sents[5:8] or sents[1:3] or sents[:1]
...
def extract_candidate_nouns(text, entities, limit=12):
    words=re.findall(r"[A-Za-zА-Яа-яЁё]{3,}", text); candidates=[]
    for w in words:
        wl=w.lower()
...
def gen_hidden_tags(title, body, entities, min_tags=3, max_tags=5):
    text_l=(title+" "+body).lower(); thematic=[]
    def tadd(x): 
...
def fit_title_in_box(draw, text, font_path, box_w, box_h, start=66, min_s=28, line_gap=8, max_lines=5):
    ...
    font=ImageFont.truetype(font_path,min_s); lines=wrap_text_by_width(draw,text,font,box_w,max_lines=max_lines)
    return font,lines

def draw_title_card(title_text, src_domain, tzname, event_dt_utc, post_dt_utc):
    W,H=1080,540; bg=random_gradient(W,H)
    overlay=Image.new("RGBA",(W,H),(0,0,0,0))
    ImageDraw.Draw(overlay).rounded_rectangle([40,110,W-40,H-90], radius=28, fill=(0,0,0,118))
    bg=Image.alpha_composite(bg.convert("RGBA"),overlay).convert("RGB"); d=ImageDraw.Draw(bg)
    path_bold="/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    path_reg ="/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    from PIL import ImageFont
    f_brand=ImageFont.truetype(path_bold,34); f_time=ImageFont.truetype(path_reg,22); f_small=ImageFont.truetype(path_reg,20)
    ...
    box_x,box_y=72,150; box_w,box_h=W-2*box_x, H-box_y-120
    f_title,lines=fit_title_in_box(d,(title_text or "").strip(),path_bold,box_w,box_h, start=66,min_s=30,max_lines=5)
    y=box_y
    for ln in lines:
        d.text((box_x,y),ln,font=f_title,fill=(255,255,255)); y+=f_title.getbbox("Ag")[3]+8
...
def build_caption(title, p1,p2,p3, link, hidden_tags, event_dt_utc, post_dt_utc):
    title=clamp(title,200); dom=root_domain(link) if link else None
    tz=ZoneInfo(TIMEZONE); ev=event_dt_utc.astimezone(tz).strftime("%d.%m %H:%M"); po=post_dt_utc.astimezone(tz).strftime("%d.%m %H:%M")
    body=f"{p1}\n\n{p2}\n\n{p3}"
    ...
    cap="\n".join(parts)
    if len(cap)>1024:
        over=len(cap)-1024+3; p3=clamp(p3[:-min(over,len(p3))],300)
...
def send_photo(photo_bytes, caption):
    if not BOT_TOKEN: raise RuntimeError("Нет BOT_TOKEN")
    url=f"https://api.telegram.org/bot{BOT_TOKEN}/sendPhoto"
    files={"photo":("cover.png",photo_bytes,"image/png")}
    data={"chat_id":CHANNEL_ID,"caption":caption,"parse_mode":"Markdown"}
    r=requests.post(url,files=files,data=data,timeout=30); print("Telegram status:", r.status_code, r.text[:200]); r.raise_for_status(); return r.json()

# ====== Сбор фидов ======
def collect_entries():
    items=[]
    for feed_url in RSS_FEEDS:
        try: fp=feedparser.parse(feed_url)
        except Exception: continue
        for e in fp.entries or []:
            link=getattr(e,"link","") or ""; title=(getattr(e,"title","") or "").strip()
            summary=clean_html(getattr(e,"summary",getattr(e,"description",""))); ts=getattr(e,"published",getattr(e,"updated","")) or ""
            try:
                dt=dtparse.parse(ts); dt=dt.replace(tzinfo=timezone.utc) if not dt.tzinfo else dt.astimezone(timezone.utc)
            except Exception:
                dt=datetime(1970,1,1,tzinfo=timezone.utc)
            if dt.year<MIN_EVENT_YEAR: continue
            uid=hashlib.sha256((link+"|"+title+"|"+ts).encode("utf-8")).hexdigest()
            items.append({"feed":feed_url,"link":link,"title":title or "(no title)","summary":summary,"ts":ts,"dt":dt,"uid":uid})
    return items
...
def process_item(item, now_utc):
    ...
    # лог в history для дайджеста
    append_history({
        "uid": item["uid"], "title": title_ru, "link": link, "event_utc": event_dt.isoformat(),
        "posted_utc": now_utc.isoformat(), "tags": hidden_tags
    })
    print(f"Posted: {title_ru[:80]} | event={event_dt.isoformat()}"); return resp
...
def main():
    state=load_state(); posted=set(state.get("posted_uids", []))
    items=collect_entries()
    if not items: print("No entries."); return
    ...
    if not to_post: print("Nothing new in fresh window."); return
    for it in to_post:
        try:
            process_item(it, now_utc); posted.add(it["uid"]); time.sleep(1.0)
        except Exception as e: print("Error sending:", e)
    state["posted_uids"]=list(trim_posted(posted)); save_state(state)
